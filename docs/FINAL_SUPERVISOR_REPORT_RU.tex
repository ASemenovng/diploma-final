...
Практическая исследовательская часть завершена. Реализованы:
\begin{itemize}
    \item оптимизированная 3-limb арифметика MNT4-753 и MNT6-753;
    \item полное on-chain MNT4-сопряжение как reference baseline;
    \item наивная модель стоимости Tate pairing;
    \item direct residue verifier по статье \emph{On Proving Pairings};
    \item KZG- и Merkle/FRI-направления для проверки трассы цикла Миллера;
    \item предварительный MNT4/MNT6 cycle-native accounting;
    \item исследовательская реализация lollipop-305 с 2-limb арифметикой.
\end{itemize}

Главный результат имеет граничный характер: без новых системных возможностей
нельзя одновременно получить малую стоимость on-chain и малую стоимость
off-chain proof-слоя простым переносом существующей арифметики между слоями.
...
\end{verbatim}
Дополнительно нужны переносы, нормализация и условное вычитание модуля. Поэтому
значение около $2{,}959$ gas/op является не случайной неэффективностью, а
близким к практически достижимому уровнем для выбранного представления.
...
\begin{table}[htbp]
\centering
\caption{Интегральная лестница MNT4}
\small
...
BN254 precompile. В проекте реализован opening layer:
\[
...
То есть KZG уменьшает gas verifier-а, но возвращает исходную проблему роста
constraints.
...
Вывод: Merkle/FRI может быть дешевле Article640 fixed-shards verifier, но не
достигает целевого уровня ниже $60$ млн gas. Стоимость переносится в calldata,
Merkle hashing и раскрытия таблиц.
...
Для иллюстрации примем:
\[
    \text{ETH price}=3000\text{ USD}.
\]
Тогда:
\[
    \text{cost}_{USD}=
    \Gas\cdot \text{gas price}_{gwei}\cdot10^{-9}\cdot3000.
\]

\begin{table}[htbp]
\centering
\caption{Иллюстративная стоимость on-chain вызова}
\small
\begin{tabular}{l r r r}
\toprule
Режим & Gas & 5 gwei & 20 gwei \\
\midrule
MNT4 prepared sparse & 79,588,799 & \$1,194 & \$4,775 \\
MNT4 Article640 residue & 93,734,789 & \$1,406 & \$5,624 \\
MNT6 Article640 residue & 172,004,717 & \$2,580 & \$10,320 \\
lollipop full research-cycle & 133,397,213 & \$2,001 & \$8,004 \\
\bottomrule
\end{tabular}
\end{table}

Эти числа не являются прогнозом стоимости транзакции: цена ETH и gas price
меняются. Таблица показывает порядок расходов.
