...
| MNT6 packed Miller, FE и bool equation verifier реализованы | `implementations/article640_mnt6` | `forge test -vv`, Rust fixture | fixed-shards equation с проверкой G1 `226 073 973 function gas` | Используется полная оптимизированная FE: короткий MNT4-style `c`-путь не переносится автоматически | Закрыто для fixed-cache модели |
