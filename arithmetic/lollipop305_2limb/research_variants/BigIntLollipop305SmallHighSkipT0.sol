...
/// @notice Экспериментальный вариант lollipop-305: малые старшие слова и удаление лишних записей t0.
library BigIntLollipop305SmallHighSkipT0 {
        ...
        returns (uint256 r0, uint256 r1)
    {
        assembly ("memory-safe") {
            ...
            t2 := 0
            {
                ...
                {
                    // u == a1 and b1 are high limbs < 2^49, so u*b1 < 2^98 and hi is zero.
                    let lo := mul(u, b1)
                    t1 := add(t1, lo)
                    let c := lt(t1, lo)
                    t2 := add(t2, c)
    ...
    function montSqr2(uint256 a0, uint256 a1) internal pure returns (uint256 r0, uint256 r1) {
        assembly ("memory-safe") {
            ...
            t2 := 0
            {
                ...
                {
                    // a1 is a high limb < 2^49, so a1*a1 < 2^98 and hi is zero.
                    let lo := mul(a1, a1)
                    t1 := add(t1, lo)
                    let c := lt(t1, lo)
                    t2 := add(t2, c)
