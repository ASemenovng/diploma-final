...
/// @notice Арифметика базового поля исследовательского прототипа lollipop-305-158.
        ...
        returns (uint256 r0, uint256 r1)
    {
        assembly ("memory-safe") {
            ...
            t2 := 0
            {
                ...
                {
                    // Старшие слова ограничены P_1 < 2^49, поэтому a1*b1 < 2^98 и старшая половина произведения равна нулю.
                    let lo := mul(u, b1)
                    t1 := add(t1, lo)
                    let c := lt(t1, lo)
                    t2 := add(t2, c)
    ...
    function montSqr2(uint256 a0, uint256 a1) internal pure returns (uint256 r0, uint256 r1) {
        assembly ("memory-safe") {
            ...
            t2 := 0
            {
                ...
                {
                    // Старшее слово ограничено P_1 < 2^49, поэтому a1*a1 < 2^98 и старшая половина произведения равна нулю.
                    let lo := mul(a1, a1)
                    t1 := add(t1, lo)
                    let c := lt(t1, lo)
                    t2 := add(t2, c)
